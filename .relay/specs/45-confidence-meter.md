# ConfidenceMeter.swift — Complete Implementation

**File path:** `trail-viewer/Sources/Components/ConfidenceMeter.swift`
...
- Uses `Theme.yellowLight` to `Theme.blue` gradient for the fill bar, matching the notebook aesthetic
- Serif `chapterTitle` (26pt) for the large percentage number gives it a book-like feel
- `Capsule()` clip shape provides rounded ends on both track and fill
- Spring animation (0.6s response) for smooth fill transitions
- Value is clamped to 0.0...1.0 internally
- Compact mode caps bar width at 80pt for inline use
- All colors/spacing/typography reference the existing Design system
